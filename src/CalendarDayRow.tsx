...
function DateCell(props: CalendarDayRowProps) {
  return (
    <td
      ...
      rowSpan={props.events.length <= 0 ? 1 : props.events.length}
    >
...
export default function CalendarDayRow(props: CalendarDayRowProps) {
  ...
  const rowClass =
    today.getDay() === props.date.getDay() ? "table-warning" : "";
  ...
  return (
    <React.Fragment>
      {props?.events?.map(function (event: Event, index: Number) {
        ...
        return (
          <tr className={rowClass} key={event.id}>
            ...
            <td className="calendar-view-timespan">
              <div>
                {formatDateTime(event.start?.dateTime, props.timeFormat)} -{" "}
                {formatDateTime(event.end?.dateTime, props.timeFormat)}
              </div>
            </td>
            <td>
